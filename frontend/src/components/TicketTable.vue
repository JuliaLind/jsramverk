...
const trainnumbers = [...new Set(data.map((item) => {
...
<template>
    ...
    <NewTicket v-if="show" :codes="reasoncodes" :trainnumbers="trainnumbers" @form-submitted="updateTickets(); toggleNewForm()" ref="show"/>
    <SingleTicket v-for="ticket in tickets" :key="ticket._id" :codes="reasoncodes" :trainnumbers="trainnumbers" :current="ticket" @form-submitted="updateTickets()"/>
    </div>
</template>
